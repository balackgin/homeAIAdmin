...
import {
  ...
  reducer,
} from 'src/features/samples/redux/fetchCaseRenderData';

describe('samples/redux/fetchCaseRenderData', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('correct action by fetchCaseRenderData', () => {
    expect(fetchCaseRenderData()).to.have.property('type', SAMPLES_FETCH_CASE_RENDER_DATA_BEGIN);
  });

  it('returns correct action by dismissFetchCaseRenderDataError', () => {
    expect(dismissFetchCaseRenderDataError()).to.have.property('type', SAMPLES_FETCH_CASE_RENDER_DATA_DISMISS_ERROR);
  });
  ...
  it('dispatches SAMPLES_FETCH_CASE_RENDER_DATA_SUCCESS action when succeeded', () => {
  ...
  it('dispatches SAMPLES_FETCH_CASE_RENDER_DATA_FAILURE action when failed', () => {
  ...
  // reducer tests
  it('handles action type SAMPLES_FETCH_CASE_RENDER_DATA_BEGIN correctly', () => {
  ...
  it('handles action type SAMPLES_FETCH_CASE_RENDER_DATA_SUCCESS correctly', () => {
    const prevState = { fetchCaseRenderDataPending: true };
    const state = reducer(
      prevState,
      { type: SAMPLES_FETCH_CASE_RENDER_DATA_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.fetchCaseRenderDataPending).to.be.false;
  });

  it('handles action type SAMPLES_FETCH_CASE_RENDER_DATA_FAILURE correctly', () => {
    const prevState = { fetchCaseRenderDataPending: true };
    const state = reducer(
      prevState,
      { type: SAMPLES_FETCH_CASE_RENDER_DATA_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.fetchCaseRenderDataPending).to.be.false;
  ...
  it('handles action type SAMPLES_FETCH_CASE_RENDER_DATA_DISMISS_ERROR correctly', () => {
